...
if __name__ == "__main__":
	...
	print subsplit
	for i in range(0,len(subsplit)):
		if subsplit[i][0]=='-':
			filelist[int(subsplit[i][1])-1]=''
		elif subsplit[i][0]=='+':
			j=i
			string=subsplit[i][2]+'\n'
			while(j+1<len(subsplit) and subsplit[j+1][1]==subsplit[i][1]):
				string.join(subsplit[j+1][2]+'\n')
				j+=1
			i=j-1
			filelist[int(subsplit[i][1])-1]=string
	print filelist
